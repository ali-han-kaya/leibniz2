...
5. apps/dashboard-next/app/globals.css must import design-system/tailwind.css
   (the generated Tailwind v4 bridge) and must NOT re-define any token
   name from tokens.css :root in its own :root block (shadow = drift).
6. design-system/tailwind.css :root block must equal tokens.css :root
   verbatim (the file is GENERATED by scripts/generate_tailwind.py;
   hand-edits break the bridge).
...
def main() -> int:
    ...
    # 5. dashboard-next must consume the generated bridge, not re-define tokens
    shadowed: list[str] = []
    next_ok = NEXT_GLOBALS.exists()
    if next_ok:
        next_text = NEXT_GLOBALS.read_text(encoding="utf-8")
        # must be an actual @import statement, not just a comment mention
        has_bridge_import = re.search(
            r'@import\s+["\'][^"\']*design-system/tailwind\.css["\']', next_text
        )
        if not has_bridge_import:
            drift.append(
                "apps/dashboard-next/app/globals.css does not import "
                "design-system/tailwind.css (bridge import missing)"
            )
        next_root = vars_in_block(next_text, r":root")
        shadowed = sorted(set(css_dark) & set(next_root))
        if shadowed:
            drift.append(
                "apps/dashboard-next/app/globals.css re-defines token(s) from "
                f"tokens.css :root — shadowed: {', '.join(shadowed)}"
            )
    ...
    bridge_root = (
        ...
        if TAILWIND_BRIDGE.exists()
        else {}
    )
    if bridge_root != css_dark:
        drift.append(
            "design-system/tailwind.css :root != tokens.css :root — regenerate: "
            "python3 design-system/scripts/generate_tailwind.py"
        )
    ...
    print(f"OK — import present, tokens.css :root {len(css_dark)} + light {len(css_light)} vars, "
          ...
          f"dashboard-next bridge {'OK' if next_ok and not shadowed else 'n/a'}")
    return 0
