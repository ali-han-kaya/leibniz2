...
if (fs.existsSync(BUDGET_PATH)) {
  try {
    ...
    console.log(`budget/index.json okunamadı: ${e.message} — degrade null`);
  }
}
let pc = null;
if (fs.existsSync(PC_PATH)) {
  try {
    pc = JSON.parse(fs.readFileSync(PC_PATH, "utf8"));
  } catch (e) {
    console.log(
      `precommit_findings/PRECOMMIT_RAPORU.json okunamadı: ${e.message} — degrade null`
    );
  }
}
let k0 = null;
if (fs.existsSync(K0_PATH)) {
  try {
    k0 = JSON.parse(fs.readFileSync(K0_PATH, "utf8"));
  } catch (e) {
    console.log(`k0_findings.json okunamadı: ${e.message} — degrade null`);
  }
}
let lineage = null;
if (fs.existsSync(LINEAGE_PATH)) {
  try {
    ...
    console.log(`lineage_findings.json okunamadı: ${e.message} — degrade null`);
  }
}
let klayers = null;
if (fs.existsSync(KLAYERS_PATH)) {
  try {
    klayers = JSON.parse(fs.readFileSync(KLAYERS_PATH, "utf8"));
  } catch (e) {
    console.log(`klayers.json okunamadı: ${e.message} — degrade null`);
  }
}
const hasReproManifest = fs.existsSync(REPRO_MANIFEST_PATH);
const k10Verdict = fs.existsSync(K10_VERDICT_PATH)
  ? fs.readFileSync(K10_VERDICT_PATH, "utf8").trim()
  : null;
...
let budgetBadge;
const cliOverrides =
  budget && budget.cli_overrides ? budget.cli_overrides.overrides || [] : [];
const hasCliOverride =
  budget &&
  budget.cli_overrides &&
  budget.cli_overrides.warning &&
  cliOverrides.length;
if (budget) {
  ...
  if (failures.length) {
    ...
    if (hasCliOverride) {
      budgetLines.push("");
      budgetLines.push(
        "🔧 **CLI override tespit edildi — bütçe kalkanı dosya " +
    ...
    for (const r of runs) {
      budgetLines.push(
        `- **${r.source || "verify"}**: $${r.estimated_usd} / $${r.limit} (~${r.tokens_est} token)`
      );
    }
    if (hasCliOverride) {
      budgetLines.push("");
...
if (!hasReproManifest || !k10Verdict) {
  ...
  reproLines.push(
    "- manifest.json + manifest.sha256 bundle bütünlüğü K10 ile doğrulandı"
  );
...
const hasReproFindings =
  !hasReproManifest || !k10Verdict || k10Verdict !== "PASS";
...
if (!hasAnyFindings) {
  ...
  if (existing) {
    ...
    console.log(
      `Bulgular çözüldü — bayat yorum kaldırıldı: comment_id=${existing.id}`
    );
  } else {
