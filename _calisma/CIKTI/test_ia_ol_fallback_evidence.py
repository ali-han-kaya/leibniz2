...
verify_delivery._archive_with_fallback zincirini (IA → HT → OL → GB) koşar;
beklenen V5s durumu — Xunzi → hathitrust, diğer 4 → openlibrary, TÜMÜ PASS —
bozulursa test FAIL (fail-closed). Ayrıca kaynak anahtarları/ht_ids varlığını
ve offline determinizmi kapılar. stdlib unittest — ek bağımlılık yok.
"""
import pathlib
import sys
import unittest
...
class TestOfflineEvidence(unittest.TestCase):
    ...
    def test_sources_match_v5s(self):
        """V5s: Xunzi → hathitrust; Fine/Lagrée/Millican/Schmitt → openlibrary."""
        ...
        for key in ev.OL_SOURCES:
            self.assertEqual(results[key]["source"], "openlibrary",
                             f"{key} OL fallback kaynağı olmalı")
    ...
    def test_evidence_shows_fallback_chain(self):
        """Detay zinciri belgelemeli: IA kapsam dışı + HT kayıt yok + OL yanıtı."""
        results = {r["key"]: r for r in ev.collect_evidence(offline=True)}
        for key in ev.OL_SOURCES:
