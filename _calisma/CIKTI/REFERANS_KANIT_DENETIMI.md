...
Fallback sırası da değişti: `_archive_fallback` artık **HathiTrust'ı OpenLibrary'den ÖNCE** dener — HT kaydı başlıkla birebir katalog kanıtıdır, OL arama eşleşmesinden daha güçlü; HT'de kayıt yoksa (0 kayıt, ~1 sn) LoC devreye girer (V5w: Lagrée/Millican/Schmitt → `loc` kaynağı, LoC lccn ile ulusal katalog kanıtı); LoC'de de yoksa OL fallback denenir. Canlı doğrulama: Xunzi → `hathitrust`, Lagrée/Millican/Schmitt → `loc` (V5w), Fine → `openlibrary` (hepsi PASS). **Dürüst sınır (V5w ile kapatıldı):** Lagrée/Millican/Schmitt/Fine kitapları HT kataloğunda gerçekten YOKtu (her üç identifier tipi — oclc, lccn, isbn — HT Bib API'de 0 kayıt döndürdü); V5w sonrası Lagrée/Millican/Schmitt artık LoC lccn ile ulusal katalog kanıtına sahip, yalnızca Fine 2012 OL fallback ile PASS'ta kalır.
...
**V5s (2026-08-20): "5 UNVERIFIED" öncülü bugünkü canlı koşuyla kapandı — 61/61 PASS, UNVERIFIED=0.** `--check-references` yeniden koşuldu (`by_verdict: {PASS: 61}`, 0 UNVERIFIED, 0 MISMATCH). Hedef 5 kaynağın **V5s anındaki** durumu: **Xunzi (Knoblock) → `hathitrust` PASS** (`lccn:87033578`; ayrıca `oclc:17265207` — HT kaydının kendi OCLC'si); **Fine 2012, Lagrée 1994, Millican 2002, Schmitt 1972 → `openlibrary` fallback PASS** (o tarihte LoC henüz eklenmemişti). **Güncel durum (V5w sonrası):** Lagrée/Millican/Schmitt artık `loc` kaynağıyla PASS (LoC lccn katalog kanıtı — V5w), yalnızca Fine 2012 OL fallback'te kalır. "UNVERIFIED" görünümünün kaynağı, dünkü yerel simülasyon çıktısıydı (`.freebuff/sim/verify_job/references_online.json`, 49/54); sim bugünkü kodla yeniden koşulunca 5 kaynak da PASS'e döner (refs-online VERSION JSON + sim çıktısıyla doğrulandı). OCLC→HathiTrust bağlantısı bugün canlı API yanıtıyla yeniden denendi:
...
**Canlı run doğrulaması (2026-08-21):** Aynı gün canlı CI run'ında `refs-online` artifact'ı `hathitrust=1` (Xunzi `lccn:87033578`) ve toplam **61/61 PASS** üretti. Bu koşu, V5w LoC ekleme sonrası zincirin canlı ortamda gerçekten çalıştığını kanıtlar: `by_source: {crossref 6, sep 5, openlibrary 22, archive 20, loc 3, hathitrust 1, handle 1, perseus 2}`. 56/56 yerel doğrulama (§5.3'teki `ia_ol_fallback_evidence.py --offline` çıktısı) ile bu canlı run arasındaki fark, LoC/Handle/ia_ids fallback'lerinin CI ortamında da devreye girmesidir — yerel doğrulama tek başına yeterli değildi.
...
**V5aa (2026-08-24): CI'da geçici OL timeout'ları belgelendi + outer retry eklendi.** CI push'larında OpenLibrary istekleri zaman zaman zaman aşımına uğrar: standart `push` run'larında 58/61 PASS (3 kaynak OL timeout nedeniyle UNVERIFIED); `workflow_dispatch` ile taze koşuda 61/61 PASS (timeout'lar geçici, ikinci denemede başarılı). Bu gözlem `verify_delivery.py`'ye `_ol_retry` dış retry kalkanı olarak uygulandı: `openlibrary_check` ve `openlibrary_fallback_check` çağrıları dış katmanla sarılır; zaman aşımı/connection reset/5xx gibi geçici UNVERIFIED'lar 3s bekleyip bir kez daha denenir (429/0 sonuç kalıcı olarak geçirilir). Canlı CI (`32679066922`, `564d6ac`): `archive_group: 25`, `by_source: {crossref 6, sep 5, openlibrary 22, archive 25, handle 1, perseus 2}` — OL retry bu run'da tetiklenmedi (geçici hata oluşmadı); mekanizma fail-closed korunur (retry başarısızsa UNVERIFIED kalır). Kapsam: **61/61 PASS** (değişmedi).

**V5ab (2026-08-24): OL geçici timeout'larına exponential backoff.** V5aa'nın tek-3s-deneme kalkanı, CI'daki geçici OL zaman aşımlarını daha etkin sıfırlamak için üstel geri çekilmeye yükseltildi: `_http_get` per-request retry'i doğrusal (1s/2s/3s) yerine `_backoff_delay` ile üstel (1s/2s/4s/8s, `REFERENCE_BACKOFF_CAP_S=8.0` tavan) bekleme kullanır; `_ol_retry` dış kalkanı tek deneme yerine `OL_RETRY_ATTEMPTS=3` toplam denemeyle `_backoff_delay` sırasını (1s, 2s) izler. Davranış korunur: PASS/MISMATCH olduğu gibi geçer, 429/0-sonuç/kapsam-dışı kalıcı sinyallerinde retry yapılmaz, retry'ler de kalıcı sinyal dönerse erken çıkılır, tüm geçici denemeler tükenirse sonuç UNVERIFIED kalır (fail-closed — yanlış PASS yok). Yalnızca hata yolunda ek bekleme (başarı ilk denemede döner) — bütçe etkisi ~3s. Kapsam: **61/61 PASS** (değişmedi).
