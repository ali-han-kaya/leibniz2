...
function fmtTs(iso) {
  ...
  try {
    ...
    return (
      d.toLocaleTimeString() + " (" + iso.split("T")[1].slice(0, 5) + " UTC)"
    );
  } catch (e) {
    return iso;
  }
...
function renderLiveFindings() {
  ...
  if (p0s.length) {
    html += p0s
      .map(
        (f) =>
          `<div style=\"color:var(--err);margin:2px 0\">🔴 <b>P0</b> ${escapeHTML(f.line)}</div>`
      )
      .join("");
  }
  if (p1s.length) {
    html += p1s
      .map(
        (f) =>
          `<div style=\"color:var(--warn);margin:2px 0\">🟡 <b>P1</b> ${escapeHTML(f.line)}</div>`
      )
      .join("");
  }
...
function colorizeLine(line) {
  if (
    /^\[P0\]|^\[FAIL\]|^SONUÇ: FAIL|FAIL \(P0=|^Error|Exception|^Traceback|^HATA/.test(
      line
    )
  )
  ...
  if (
    /\[K8\]|\bUNSAT\b|\bSAT\b|^\[PASS\]|^SONUÇ: PASS|^SONUÇ: TÜMÜ PASS|^Verdict: PASS/.test(
      line
    )
  )
...
function updateBudgetOverBanner() {
  ...
  const over = rows.filter(
    (r) => r.budget_usd != null && r.budget_usd > BUDGET_LIMIT
  );
  const liveOver =
    budgetState &&
    budgetState.est != null &&
    budgetState.limit &&
  ...
  if (over.length) {
    ...
    bits.push(
      `${over.length} run limiti aştı · en yüksek $${worst.budget_usd.toFixed(2)} @ ${t}`
    );
  }
  if (liveOver) {
    bits.push(
  ...
  if (det) {
    ...
    const liveLine = liveOver
      ? "CANLI $" +
        ...
        " > limit $" +
        fmtLimit(budgetState.limit)
      : "";
    det.innerHTML =
      budgetOverDetailRows(over) +
...
function sealHashFromSnapshot(d) {
  ...
  return (
    (snap && typeof snap.stripped === "string" && snap.stripped) ||
...
function renderTrend(rows) {
  ...
  const yD = (v) =>
    v == null || !isFinite(v) ? null : PT + ih - (ih * v) / maxDur;
  const yB = (v) =>
    v == null || !isFinite(v) ? null : PT + ih - (ih * v) / maxBud;
  const yZ = (v) =>
    v == null || !isFinite(v) ? null : PT + ih - (ih * v) / maxZ;
  ...
  for (let g = 0; g <= maxP; g++) {
    const gy = yP(g);
    parts.push(
      `<line x1="${PL}" y1="${gy}" x2="${plotRight}" y2="${gy}" stroke="#21262d" stroke-width="1"/>`
    );
    parts.push(
      `<text x="${PL - 6}" y="${gy + 4}" fill="#8b949e" font-size="10" text-anchor="end">${g}</text>`
    );
  }
  // P0 (kırmızı), P1 (sarı)
  if (rows.some((r) => (r.p0 || 0) > 0)) {
    parts.push(
      `<polyline fill="none" stroke="#f85149" stroke-width="2" points="` +
        rows.map((r, i) => `${x(i)},${yP(r.p0 || 0)}`).join(" ") +
        `"/>`
    );
  }
  parts.push(
    `<polyline fill="none" stroke="#d29922" stroke-width="2" points="` +
  ...
  const durPts = rows
    .map((r, i) => {
      const y = yD(r.duration_s);
      return y == null ? null : `${x(i)},${y}`;
    })
    .filter(Boolean);
  if (durPts.length) {
    parts.push(
      `<polyline fill="none" stroke="#58a6ff" stroke-width="2" stroke-dasharray="4 3" points="${durPts.join(" ")}"/>`
    );
  }
  ...
  const budPts = rows
    .map((r, i) => {
      const y = yB(r.budget_usd);
      return y == null ? null : `${x(i)},${y}`;
    })
    .filter(Boolean);
  if (budPts.length) {
    parts.push(
      `<polyline fill="none" stroke="#bc8cff" stroke-width="2" points="${budPts.join(" ")}"/>`
    );
  }
  ...
  if (limsVary) {
    ...
    if (stepPts.length) {
      parts.push(
        `<polyline fill="none" stroke="#ffa657" stroke-width="2" points="${stepPts.join(" ")}"/>`
      );
    }
  }
  ...
  if (buds.length) {
    ...
    parts.push(
      `<line x1="${PL}" y1="${limitY}" x2="${plotRight}" y2="${limitY}" stroke="#ffa657" stroke-width="1.5" stroke-dasharray="6 4"/>`
    );
    parts.push(
      `<text x="${budAxisX + 6}" y="${limitY - 3}" fill="#ffa657" font-size="9">${offScale ? limitLbl + " ↑" : limitLbl}</text>`
    );
    ...
    rows.forEach((r, i) => {
      if (r.budget_usd != null && r.budget_usd > BUDGET_LIMIT) {
        ...
        if (yy != null) {
          parts.push(
            `<rect x="${x(i) - 3}" y="${yy - 3}" width="6" height="6" fill="#f85149"/>`
          );
        }
      }
    });
  ...
  const zPassPts = rows
    .map((r, i) => {
      const y = yZ(r.z3_passed);
      return y == null ? null : `${x(i)},${y}`;
    })
    .filter(Boolean);
  if (zPassPts.length) {
    parts.push(
      `<polyline fill="none" stroke="#3fb950" stroke-width="2" points="${zPassPts.join(" ")}"/>`
    );
  }
  const zFailPts = rows
    .map((r, i) => {
      const y = yZ(r.z3_failed);
      return y == null ? null : `${x(i)},${y}`;
    })
    .filter(Boolean);
  if (zFailPts.length) {
    parts.push(
      `<polyline fill="none" stroke="#f85149" stroke-width="2" stroke-dasharray="3 3" points="${zFailPts.join(" ")}"/>`
    );
  }
  // K9 Lean PASS oranı (%) — beşinci eksen: lean_ok true → 100, false → 0, null → skip
  const leans = rows.map((r) =>
    r.lean_ok === true ? 100 : r.lean_ok === false ? 0 : null
  );
  const leanVals = leans.filter((v) => v != null && isFinite(v));
  const hasLean = leanVals.length > 0;
  const maxLean = hasLean ? 100 : 1;
  const yL = (v) =>
    v == null || !isFinite(v) ? null : PT + ih - (ih * v) / maxLean;
  const leanPts = rows
    .map((r, i) => {
      const y = yL(leans[i]);
      return y == null ? null : `${x(i)},${y}`;
  ...
  if (leanPts.length) {
    parts.push(
      `<polyline fill="none" stroke="#e055d2" stroke-width="2.5" stroke-dasharray="5 2" points="${leanPts.join(" ")}"/>`
    );
  }
  ...
  rows.forEach((r, i) => {
    ...
    if (r.lean_ok === true) {
      ...
      parts.push(
        `<rect x="${cx - 3}" y="${ly - 3}" width="6" height="6" fill="#f85149"/>`
      );
    }
  });
  ...
  if (durs.length) {
    parts.push(
      `<line x1="${durAxisX}" y1="${PT}" x2="${durAxisX}" y2="${PT + ih}" stroke="#58a6ff" stroke-width="1" opacity="0.5"/>`
    );
    parts.push(
      `<text x="${durAxisX + 6}" y="${PT + 4}" fill="#58a6ff" font-size="9">${maxDur.toFixed(0)}s</text>`
    );
    parts.push(
      `<text x="${durAxisX + 6}" y="${PT + ih + 2}" fill="#58a6ff" font-size="9">0s</text>`
    );
  }
  if (buds.length) {
    parts.push(
      `<line x1="${budAxisX}" y1="${PT}" x2="${budAxisX}" y2="${PT + ih}" stroke="#bc8cff" stroke-width="1" opacity="0.5"/>`
    );
    parts.push(
      `<text x="${budAxisX + 6}" y="${PT + 4}" fill="#bc8cff" font-size="9">$${maxBud.toFixed(2)}</text>`
    );
    parts.push(
      `<text x="${budAxisX + 6}" y="${PT + ih + 2}" fill="#bc8cff" font-size="9">$0</text>`
    );
  }
  if (ztots.length) {
    parts.push(
      `<line x1="${zAxisX}" y1="${PT}" x2="${zAxisX}" y2="${PT + ih}" stroke="#3fb950" stroke-width="1" opacity="0.5"/>`
    );
    parts.push(
      `<text x="${zAxisX + 6}" y="${PT + 4}" fill="#3fb950" font-size="9">${maxZ} PASS</text>`
    );
    parts.push(
      `<text x="${zAxisX + 6}" y="${PT + ih + 2}" fill="#3fb950" font-size="9">0</text>`
    );
  }
  if (hasLean) {
    parts.push(
      `<line x1="${leanAxisX}" y1="${PT}" x2="${leanAxisX}" y2="${PT + ih}" stroke="#e055d2" stroke-width="1" opacity="0.5"/>`
    );
    parts.push(
      `<text x="${leanAxisX + 6}" y="${PT + 4}" fill="#e055d2" font-size="9">100%</text>`
    );
    parts.push(
      `<text x="${leanAxisX + 6}" y="${PT + ih + 2}" fill="#e055d2" font-size="9">0%</text>`
    );
  }
  ...
  rows.forEach((r, i) => {
    ...
    parts.push(
      `<circle cx="${x(i)}" cy="${yP(Math.max(r.p0 || 0, r.p1 || 0))}" r="2.5" fill="${c}"/>`
    );
  });
  ...
  const pick = [0, Math.floor((n - 1) / 2), n - 1].filter(
    (v, i, a) => a.indexOf(v) === i
  );
  pick.forEach((i) => {
    const r = rows[i];
    ...
    parts.push(
      `<text x="${x(i)}" y="${H - 12}" fill="#8b949e" font-size="10" text-anchor="middle">${lbl}</text>`
    );
  });
  ...
  legend.textContent =
    "Kırmızı = P0 · Sarı = P1 · Yeşil nokta = PASS · Mavi (kesikli) = duration_s · Mor = budget_usd · " +
    "Yeşil çizgi = Z3 PASS · Kırmızı (noktalı) = Z3 FAIL · " +
    "Pembe (kesikli) = Lean PASS oranı (son: " +
    leanPct +
    ") · " +
    "Turuncu (kesikli) = güncel limit $" +
    fmtLimit(BUDGET_LIMIT) +
    " (config'ten) — altı güvenli, üstü (kırmızı kare) aşım" +
    (limsVary ? " · Turuncu (düz) = per-run limit (config değişimi)" : "") +
    " · aralık: " +
    (rows[0].ts ? new Date(rows[0].ts).toLocaleString() : "?") +
    " → " +
...
function budgetLimitNote(v, lim) {
  ...
  return v > l
    ? " (limit " + limTxt + " üstünde — AŞIM)"
    : " (limit " + limTxt + " altında)";
}
...
function trendTipHeader(r) {
  ...
  return [
    ...
    '<span class="' +
      budgetTipColor(r.budget_usd, lim) +
      '">budget  : ' +
      fmtTrendBudget(r.budget_usd) +
      budgetLimitNote(r.budget_usd, lim) +
      "</span>",
...
function showTrendTip(i, ev) {
  ...
  if (!r) return;
  const verdict =
    (r.p0 || 0) > 0 ? "FAIL (P0)" : (r.p1 || 0) > 0 ? "WARN (P1)" : "PASS";
  const tip = $("tip");
  let leanLine = "lean    : —";
  if (r.lean_ok === true) leanLine = "lean    : PASS (100%)";
    ...
    leanLine =
      "lean    : FAIL (0%)" +
      (r.lean_detail ? " — " + escapeHTML(r.lean_detail) : "");
...
function refsTrendBadge(rows) {
  const have = (rows || []).filter(
    (r) => r.refs_verified != null && r.refs_total != null
  );
  ...
  return {
    cls: "warn",
    text: "kapsam eksik " + last.refs_verified + "/" + last.refs_total,
  };
}
let refsTrendCache = [];
...
function showRefsTrendTip(i, ev) {
  ...
  let lines = [
    ...
    head[3],
    "refs    : " +
      (r.refs_verified != null ? r.refs_verified : "?") +
      " / " +
...
function renderRefsTrend(rows) {
  ...
  if (!have.length) {
    svg.innerHTML = "";
    legend.textContent =
      "çevrimiçi referans verisi yok — bu run'larda --check-references veri üretmedi";
    $("refs-trend-count").textContent = "";
    const b0 = refsTrendBadge([]);
    const bEl0 = $("refs-trend-badge");
    if (bEl0) {
      bEl0.className = "badge " + b0.cls;
      bEl0.textContent = b0.text;
    }
    return;
  }
  const W = 1160,
  ...
  activeSrc.forEach((s) => {
    ...
    const pts =
      ...
      [...top]
        .reverse()
        .map((_, i) => `${x(n - 1 - i)},${y(base[n - 1 - i])}`)
        .join(" ");
    parts.push(
      `<polygon fill="${color}" fill-opacity="0.30" stroke="none" points="${pts}"/>`
    );
    for (let i = 0; i < n; i++) base[i] = top[i];
  });
  // total (mavi, kesikli) — çevrimiçi denetlenebilir girdi sayısı
  parts.push(
    `<polyline fill="none" stroke="#58a6ff" stroke-width="2" stroke-dasharray="4 3" points="` +
      have.map((r, i) => `${x(i)},${y(r.refs_total)}`).join(" ") +
      `"/>`
  );
  // verified (yeşil, dolu) — doğrulanan
  parts.push(
    `<polyline fill="none" stroke="#3fb950" stroke-width="2.5" points="` +
  ...
  have.forEach((r, i) => {
    ...
    parts.push(
      `<circle cx="${x(i)}" cy="${y(r.refs_verified)}" r="2.5" fill="${full ? "#3fb950" : "#d29922"}"/>`
    );
  });
  ...
  const pct =
    last.refs_total > 0
      ? Math.round((100 * last.refs_verified) / last.refs_total)
      : 0;
  // by_source seri lejantı (renkli ■ çipler, son run sayıları ile)
  const lastSrc = activeSrc
    .map((s) => {
    ...
    .join(" ");
  legend.innerHTML =
    "Yeşil = verified · Mavi (kesikli) = total · Kırmızı = mismatch · Son: " +
...
function configSyncBadge(s) {
  ...
  if (s.has_drift) {
    return {
      cls: "err",
      text: "✗ Schema Sync: " + vt + " (" + (s.error_count || 0) + " drift)",
    };
  }
...
function renderConfigSync(s) {
  ...
  if (body) {
    if (!s || s.available !== true) {
      body.textContent =
        "Schema Sync: veri yok (config-sync job'u henüz koşmadı)";
    } else if (s.has_drift) {
      body.innerHTML =
        (s.details || [])
          .map(
...
function renderRefsTrendDurationBudget(rows) {
  ...
  if (durs.length) {
    ...
    if (pts.length)
      parts.push(
        `<polyline fill="none" stroke="#58a6ff" stroke-width="2" stroke-dasharray="4 3" points="${pts.join(" ")}"/>`
      );
  }
  if (buds.length) {
    const pts = rows
    ...
    if (pts.length)
      parts.push(
        `<polyline fill="none" stroke="#bc8cff" stroke-width="2" points="${pts.join(" ")}"/>`
      );
  }
  ...
  if (buds.length) {
    ...
    parts.push(
      `<line x1="${PL}" y1="${ly}" x2="${plotRight}" y2="${ly}" stroke="#ffa657" stroke-width="1" stroke-dasharray="6 4" opacity="0.6"/>`
    );
  }
  ...
  if (parts.length) {
    ...
    if (legend) {
      const extra = [];
      if (durs.length)
        extra.push(`Mavi (kesikli) = duration_s (max ${maxDur.toFixed(0)}s)`);
      if (buds.length)
        extra.push(`Mor = budget_usd (max $${maxBud.toFixed(2)})`);
...
function updateZ3Bar() {
  const bar = $("z3-fill"),
    cnt = $("z3-count");
  if (!bar || !cnt) return;
  const done = z3Seen.size,
    passed = z3Passed.size,
    failed = z3Failed.size;
  bar.style.width = Math.round((done / Z3_TOTAL) * 100) + "%";
  if (failed > 0) {
    bar.className = "z3fill err";
    cnt.className = "z3count";
  } else if (passed === Z3_TOTAL) {
    bar.className = "z3fill ok";
    cnt.className = "z3count ok";
  } else {
    bar.className = "z3fill";
    cnt.className = "z3count";
  }
  cnt.textContent =
    passed + "/" + Z3_TOTAL + (failed > 0 ? " · " + failed + " FAIL" : "");
}
...
function connectStream() {
  ...
  runStreamES.addEventListener("replay-start", (e) => {
    ...
    const scls =
      sv === "PASS" ? "ok" : sv === "FAIL" || sv === "ERROR" ? "err" : "warn";
    const p0 = sum && sum.p0 != null ? sum.p0 : 0;
    const p1 = sum && sum.p1 != null ? sum.p1 : 0;
    const bud =
      sum && sum.budget_usd != null ? "$" + sum.budget_usd.toFixed(2) : "—";
    const dur =
      sum && sum.duration_s != null ? " · " + fmtDuration(sum.duration_s) : "";
    const refV = sum && sum.refs_verified != null ? sum.refs_verified : null;
    const refT = sum && sum.refs_total != null ? sum.refs_total : null;
    const refs =
      refV != null && refT != null ? " · refs " + refV + "/" + refT : "";
    const pg =
      sum && sum.pdf_pages != null ? " · " + sum.pdf_pages + " sayfa" : "";
    const tlabel =
      sum && sum.ts && sum.ts.indexOf("T") >= 0
  ...
  runStreamES.addEventListener("stdout", (e) => {
    try {
      const d = JSON.parse(e.data);
      push("stdout", d.line, d.replay);
    } catch (_) {}
  });
  runStreamES.addEventListener("stderr", (e) => {
    try {
      const d = JSON.parse(e.data);
      push("stderr", d.line, d.replay);
    } catch (_) {}
  });
...
function renderOverrideTrend(rows) {
  ...
  if (legend) {
    legend.textContent =
      OVERRIDE_COLOR_RULES.warning.label +
      " · " +
      OVERRIDE_COLOR_RULES.clean.label;
  }
  if (svg) {
    const W = 1160,
      H = 120,
    ...
    have.forEach((r, i) => {
      ...
      const color = r.warning
        ? OVERRIDE_COLOR_RULES.warning.color
...
function renderDeterminismTrend(rows) {
  ...
  if (svg) {
    ...
    const W = 1160,
      ...
      PAD = 24;
    const xAt = (i, n) =>
      n === 1 ? W / 2 : PAD + ((W - 2 * PAD) * i) / (n - 1);
    let parts = [];
    if (!have.length) {
      parts.push(
        `<text x="${W / 2}" y="${H / 2}" fill="#8b949e" font-size="12">veri yok — koşum bekleniyor</text>`
      );
    } else {
      const n = have.length;
      have.forEach((r, i) => {
      ...
      for (let i = 1; i < n; i++) {
        ...
        if (
          ...
          parts.push(
            `<line x1="${xAt(i - 1, n).toFixed(1)}" y1="${H / 2}" x2="${xAt(i, n).toFixed(1)}" y2="${H / 2}" stroke="#3fb950" stroke-width="2"/>`
          );
        }
      }
    }
...
function loadTrend(force) {
  ...
  fetch("/api/trend")
    ...
    .then((data) => {
      ...
      const dbRows =
        (data.refs_trend &&
          data.refs_trend.duration_budget &&
          data.refs_trend.duration_budget.rows) ||
        [];
...
function renderBudget(b) {
  ...
  brEl.textContent = order
    .map((k) => k + "=" + (ratios[k] != null ? ratios[k] : "?"))
    .join(" · ");
...
function renderRefsOnline(d) {
  ...
  if (total == null) {
    ...
    $("ro-mis").textContent = "—";
    $("ro-cov").textContent = "—";
    $("ro-vt").className = "value";
    $("ro-cov").className = "value";
    sourcesEl.innerHTML =
      '<div class="muted">çevrimiçi referans denetimi bu run\'da veri üretmedi (ağ/denetim çalışmadı veya 0 kayıt)</div>';
  ...
  const srcColors = {
    ...
    url: "#8b949e",
    handle: "#8b949e",
    fallback_openlibrary: "#bc8cff",
    fallback_loc: "#79c0ff",
    fallback_hathitrust: "#79c0ff",
    diğer: "#8b949e",
  };
  const srcNames = {
    crossref: "CrossRef",
  ...
  if (!rows.length) {
    sourcesEl.innerHTML =
      '<div class="muted">kaynak kırılımı yok (by_source boş)</div>';
    // Hide cards too
  ...
  if (scEl) {
    ...
    for (const k of rows) {
      ...
      cardsHtml +=
        '<div class="card source-card" style="border-left:3px solid ' +
        clr +
        '">' +
        '<div class="label">' +
        icon +
  ...
  if (archiveGroup != null && archiveGroup > 0) {
    html +=
      `<div style="margin-bottom:6px;font-size:0.85em;color:var(--accent)">` +
  ...
  for (const k of rows) {
    ...
    html +=
      ...
      c +
      "</td>" +
      '<td style="font-family:monospace" class="ts">' +
      w +
      "%</td>" +
...
function renderConfigDiff(cd, ts) {
  ...
  if (!diffs || !diffs.length) {
    bodyEl.innerHTML =
      '<span class="badge ok">fark yok</span> ' +
  ...
  let h =
    "<table><thead><tr><th>Field</th><th>Raw</th><th>Effective</th><th>Neden</th></tr></thead><tbody>";
  for (const r of diffs) {
    ...
    h +=
      ...
      "</code></td>" +
      "<td><code>" +
      escapeHTML(fmtVal(r.raw)) +
      "</code></td>" +
      "<td><code>" +
      escapeHTML(fmtVal(r.effective)) +
      "</code></td>" +
      '<td><span class="badge ' +
      cls +
      '">' +
...
function renderCliOverride(data) {
  ...
  if (!items.length) {
    bodyEl.innerHTML =
      '<span class="badge ok">CLI override YOK</span> ' +
  ...
  if (!overrides.length) {
    ...
    bodyEl.innerHTML =
      ...
      '<span class="muted">' +
      items.length +
      " anahtar CLI verildi ama dosya değeriyle aynı</span>";
    return;
  }
  let h =
    '<span class="badge warn">CLI override VAR (' +
    overrides.length +
    ")</span>";
  h +=
    '<table style="margin-top:8px"><thead><tr><th>Anahtar</th>' +
  ...
  for (const [key, v] of overrides) {
    h +=
      ...
      "</code></td>" +
      "<td><code>" +
      escapeHTML(fmtVal(v.file_value)) +
      "</code></td>" +
      '<td><code style="color:var(--warn)">' +
      escapeHTML(fmtVal(v.effective)) +
      "</code></td></tr>";
  }
  h += "</tbody></table>";
  bodyEl.innerHTML = h;
...
function renderMirrorSync(data) {
  ...
  if (m == null) {
    bodyEl.innerHTML =
      '<span class="muted">K17 mirror check bu run\u0027da koşmadı (--check-mirror gerekli)</span>';
    return;
  }
  const exit = m.exit;
  const ok = m.ok === true;
  ...
  let h =
    ...
    badgeTxt +
    "</span> " +
    '<span class="muted">K17 exit=' +
    (exit != null ? exit : "—") +
    (auto ? " · auto_synced ✓ (before=" + m.before_exit + ")" : "") +
    "</span>";
  h +=
  ...
  if (stale.length) {
    h +=
      '<div style="margin-top:8px;font-size:12px;font-weight:600;color:var(--warn)">' +
      "BAYAT/EKSİK dosyalar (" +
      stale.length +
      "):</div>" +
    ...
    h +=
      '<div style="margin-top:6px;font-size:12px;color:var(--ok)">' +
...
function renderPatternDrift(data) {
  const bodyEl = $("pattern-drift-body"),
    tsEl = $("pattern-drift-ts");
  if (!bodyEl) return;
  const ts = data && data.ts;
  if (tsEl) tsEl.textContent = ts ? fmtTs(ts) : "";
  ...
  if (drift == null) {
    bodyEl.innerHTML =
      '<span class="muted">pattern drift kontrolü bu run\u0027da çalışmadı</span>';
    return;
  }
  if (drift === "PASS") {
    bodyEl.innerHTML =
...
function _cls(st) {
  return st === "PASS" ? "ok" : st === "FAIL" ? "err" : "warn";
}
function _ico(st) {
...
function renderKLayers(ls, z3p, z3t, z3f, leanOk, leanDetail) {
  ...
  for (const k of K_ALL) {
    ...
    if (k === "K8" && z3p != null) {
      ...
      const failed = z3f || 0;
      title +=
        " (" + z3p + "/" + tot + (failed > 0 ? " FAIL:" + failed : "") + ")";
    }
    if (k === "K9") {
      if (leanOk === true) title += " PASS";
      else if (leanOk === false)
        title += " FAIL" + (leanDetail ? " — " + leanDetail : "");
    }
    let kcls = _cls(st);
...
function renderHookEnv(rows) {
  ...
  let html =
    "<table><thead><tr><th>Tool</th><th>Current version</th>" +
  ...
  for (const tool of HOOK_TOOLS) {
    ...
    html +=
      "<tr><td><code>" +
      (HOOK_LABELS[tool] || tool) +
      "</code></td>" +
      "<td><code>" +
      (s ? escapeHTML(shortVer(s.v)) : "—") +
      "</code></td>" +
      "<td>" +
      (n
        ? '<span class="badge warn">' + n + "</span>"
  ...
  if (transitions.length) {
    html +=
      '<div class="ts" style="margin-top:8px">Sürüm değişiklikleri (yeni → eski):</div>';
    const rev = transitions.slice().reverse().slice(0, 20);
    for (const t of rev) {
      html +=
        '<div style="margin:3px 0"><span class="badge info">' +
        (HOOK_LABELS[t.tool] || t.tool) +
        "</span> " +
        "<code>" +
        escapeHTML(shortVer(t.from)) +
        "</code> → <code>" +
        escapeHTML(shortVer(t.to)) +
        "</code> " +
        '<span class="ts">' +
        fmtTs(t.ts) +
        "</span></div>";
    ...
    html +=
      '<div class="muted" style="margin-top:8px">henüz sürüm değişikliği yok</div>';
  }
  body.innerHTML = html;
...
function renderHookEnvTrend(rows) {
  const svg = $("he-trend"),
    legend = $("he-trend-legend");
  if (!svg) return;
  const have = (rows || []).filter(
    (r) => r.hook_env && typeof r.hook_env === "object"
  );
  heTrendCache = have;
  ...
  if (!have.length) {
    svg.innerHTML = "";
    if (legend)
      legend.textContent = "hook env sürüm verisi yok — probe edilmedi";
    return;
  }
  const W = 1160,
    H = 240,
  ...
  tools.forEach((t, ti) => {
    parts.push(
      `<text x="${PL - 8}" y="${y(ti) + 4}" fill="#8b949e" font-size="11" text-anchor="end">${HOOK_LABELS[t] || t}</text>`
    );
  });
  ...
  have.forEach((r, i) => {
    ...
    tools.forEach((t, ti) => {
      ...
      parts.push(
        `<rect x="${cx0.toFixed(2)}" y="${(y(ti) - hc / 2).toFixed(2)}" width="${wCell.toFixed(2)}" height="${hc.toFixed(2)}" fill="${color}" fill-opacity="0.85" stroke="#0d1117" stroke-width="0.5"/>`
      );
    });
  });
  ...
  if (legend) {
    ...
    have
      ...
      .forEach((env, i) => {
        for (const t of tools) {
          ...
          if (seen[t].last !== undefined && seen[t].last !== v)
            changes[t] = (changes[t] || 0) + 1;
    ...
    for (const t of tools) {
      const c = changes[t] || 0;
      partsL.push(
        (HOOK_LABELS[t] || t) + ": " + (c ? `${c} değişim` : "sabit")
      );
    }
...
function renderHookEnvDrift(matrix) {
  ...
  if (!tools.length) {
    el.innerHTML =
      '<div class="muted">matris verisi yok (hook_env prob edilmedi)</div>';
    return;
  }
  const badge =
    verdict === "DRIFT"
  ...
  let html =
    '<div style="margin-bottom:8px">Son run araçları ↔ ' +
    "<code>docs/HOOK_ENV_MATRIX.md</code> beklenen pin: " +
    badge +
    "</div>";
  html +=
    "<table><thead><tr><th>Tool</th><th>Beklenen pin (matris)</th>" +
    "<th>Gözlenen (son run)</th><th>Durum</th></tr></thead><tbody>";
  for (const t of tools) {
    const label = HOOK_LABELS[t.tool] || t.tool;
    const exp = t.expected
      ? "<code>" + escapeHTML(shortVer(t.expected)) + "</code>"
      : "—";
    const obs = t.observed
      ? "<code>" + escapeHTML(shortVer(t.observed)) + "</code>"
    ...
    if (t.status === "ok") status = '<span class="badge ok">✓</span>';
      ...
      status =
        '<span class="badge err">doc dışı — HOOK_ENV_MATRIX.md\'de yok</span>';
    html +=
      "<tr><td><code>" +
      label +
      "</code></td><td>" +
  ...
  if (tsEl && matrix && matrix.observed_ts)
    tsEl.textContent = fmtTs(matrix.observed_ts);
}
...
function applySnapshotInner(d) {
  ...
  if (d.status_board) {
    ...
    $("status-board").classList.toggle(
      "board-pass",
      !d.status_board.includes("\uD83D\uDD34")
    );
    $("status-board").classList.toggle(
      "board-fail",
      d.status_board.includes("\uD83D\uDD34")
  ...
  if (hooksEl && d.precommit_hooks && d.precommit_hooks.length > 0) {
    hooksEl.innerHTML = d.precommit_hooks
      .map((h) => {
    ...
    hooksEl.innerHTML =
      "<span style='color:var(--muted)'>⏳ hook verisi bekleniyor…</span>";
  }
  ...
  $("m-verdict").className =
    "value " + (verdict === "PASS" ? "ok" : verdict === "FAIL" ? "err" : "");
  ...
  $("m-p0p1").className =
    "value " + (p0 === 0 && p1 === 0 ? "ok" : p0 > 0 ? "err" : "warn");
  ...
  $("m-budget").textContent =
    d.budget_usd != null ? "$" + d.budget_usd.toFixed(2) : "—";
  const limTxt = "$" + fmtLimit(BUDGET_LIMIT);
  $("m-budget-sub").textContent =
    d.budget_usd != null && d.budget_usd < BUDGET_LIMIT
      ? "✓ under " + limTxt + " limit"
      : d.budget_usd != null
        ? "⚠ over " + limTxt
        : "limit " + limTxt;
  ...
  if (d.stripped_sha256) {
    ...
    $("m-meta-sub").textContent =
      "drift: " +
      (d.stripped_sha256 !== (d.expected_stripped || "").slice(0, 16)
        ? "yes"
        : "no");
  ...
  if (d.z3_passed != null) {
    ...
    const zcls =
      (d.z3_failed || 0) > 0 ? "err" : d.z3_passed >= ztot ? "ok" : "warn";
    addBadge(
      zcls,
      "K8 Z3: " +
        d.z3_passed +
  ...
  if (d.lean_ok === true) {
    ...
    addBadge(
      "err",
      "K9 Lean: FAIL" +
        (d.lean_detail ? " — " + d.lean_detail : "") +
        " (son çalıştırma)"
    );
    if (la) {
      la.classList.remove("hidden");
      la.textContent =
        "⚠ K9 Lean: FAIL — " + (d.lean_detail || "ispat zinciri kırık");
    }
  } else {
  ...
  addBadge(
    p1 === 0 ? "ok" : "warn",
    "Beth 1953: " + (p1 === 0 ? "düzeltildi" : p1 + " P1")
  );
  ...
  if (ovKeys.length) {
    const ovDetail = ovKeys
      .map(
        (k) =>
          "`" +
          k +
          "`: " +
          (cov[k].file_value != null ? cov[k].file_value : "?") +
          " → " +
          cov[k].effective
      )
      .join(" · ");
    addBadge(
      "warn",
      "Budget override: " + ovKeys.length + " (" + ovDetail + ")"
    );
  }
  ...
  if (d.stdout_short) {
    ...
    for (const line of lines) {
      const m = line.match(
        /^\s*\[(OK|FAIL|SKIP)\]\s+(CrossRef|SEP|OpenLib)\s+(\S.*?)\s+->\s+(.*)$/
      );
      if (m)
        refItems.push({
          tag: m[1],
      ...
      const lines = filtered
        ...
        .map((r) => {
          ...
          const cls =
            sv === "PASS"
              ...
              : sv === "FAIL" || sv === "ERROR"
                ...
                : "warn";
          const t =
            r.ts && r.ts.indexOf("T") >= 0
              ? r.ts.split("T")[1].slice(0, 8)
              : "?";
          const dur =
            r.duration_s != null ? r.duration_s.toFixed(0) + "s" : "—";
          const bud =
            r.budget_usd != null ? "$" + r.budget_usd.toFixed(2) : "—";
          const refs =
            r.refs_verified != null
              ? r.refs_verified + "/" + r.refs_total
              : "?/?";
          ...
          const over =
            r.budget_usd != null &&
            BUDGET_LIMIT != null &&
            isFinite(BUDGET_LIMIT) &&
            r.budget_usd > BUDGET_LIMIT;
...
function connect() {
  ...
  evtSource.addEventListener("update", (e) => {
    try {
      ...
      console.error("update parse", err);
    }
  });
  evtSource.onerror = () => {
    setLive("err");
...
(() => {
  ...
  box.addEventListener("keydown", (e) => {
    ...
    const focusables = [
      document.getElementById("z3-prev"),
  ...
  document
    .getElementById("z3-prev")
    .addEventListener("click", () => show(index - 1));
  document
    .getElementById("z3-next")
    .addEventListener("click", () => show(index + 1));
